...
```
┌────────────────────────────────────────────────────────────────────┐
│ ●  ZViewer ─── photo_01.jpg              ─  □  ✕              │ ← 自绘标题栏（毛玻璃，高约32px）
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│   ┌─── 顶部信息条（毛玻璃浮条，闲置自动隐藏） ──────────────────┐   │
│   │  photo_01.jpg  ·  4000×3000  ·  4.2 MB  · 100%  ·  3/128  ·  旅行照片 │   │
│   └──────────────────────────────────────────────────────────┘   │
│                                                                    │
│                                                                    │
│                 ┌──────────────────────────┐                       │
...
- 「解码中…」玻璃浮片贴窗口右上角（距右/顶 `16px`，沉浸模式贴顶 `16px`）：RAW/动画经 IPC 解码超过 300ms 时出现，完成或失败即隐藏
...
│  photo_01.jpg · 4000×3000 · 4.2 MB · 100% · 3/128 · 📁 旅行照片   │
└──────────────────────────────────────────────────────────────────┘
   └─ 文件名 ─┘  └── 尺寸 ──┘  └ 大小 ┘ └ 缩放 ┘ └ 位置 ┘ └ 所在文件夹 ─┘
```

- 各字段间用 `·` 分隔，分隔符用 `text-secondary` 色
- 缩放百分比实时显示（等宽数字；无图片/未加载显示 `—`）
...
| 解码中指示浮片 | RAW/动画经 IPC 解码且耗时 >300ms | 解码完成 / 失败 / 非 IPC 通道 |
